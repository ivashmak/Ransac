...
public:
    void GetLOModelScore (Estimator * const estimator,
        ...
        for (int iters = 0; iters < lo_max_iterations; iters++) {
            ...
            quality->GetModelScore(estimator, current_lo_model, input_points, points_size, *current_lo_score,
                                   current_inliers, true);
             ...
             */
            float reduce_threshold = (current_lo_model->lo_threshold * current_lo_model->lo_threshold_multiplier -
                                      current_lo_model->threshold)
        ...
        /*
         * TODO: add weights? add termination criteria with non minimal sample number?
         */
    }
};
